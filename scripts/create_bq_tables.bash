...
bq mkdef --noautodetect --source_format=NEWLINE_DELIMITED_JSON "gs://meetup_data/event_comments/*.json" ../bq_table_schema/event_comments.json > event_comments_tdf.json

jq '.+{ignoreUnknownValues:true}' rsvp_tdf.json > rsvp_tdf.json.tmp && cp rsvp_tdf.json.tmp rsvp_tdf.json
jq '.+{ignoreUnknownValues:true}' photos_tdf.json > photos.json.tmp && cp photos.json.tmp photos_tdf.json
